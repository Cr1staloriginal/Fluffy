...
class Tickets(commands.Cog):
    ...
    async def ticket(self, inter: disnake.ApplicationCommandInteraction, reason: str = None):
        ...
        overwrites = {guild.default_role: disnake.PermissionOverwrite(read_messages=False), inter.author: disnake.PermissionOverwrite(read_messages=True, send_messages=True)}
        ...
        await ch.send(f"Тикет от {inter.author.mention}. Причина: {reason or 'Не указана'}")
    ...
    @commands.slash_command(name="close_ticket", description="Закрыть текущий тикет")
    ...
    async def close_ticket(self, inter: disnake.ApplicationCommandInteraction, channel: disnake.TextChannel = None):
        ...
        await ch.send("Тикет закрывается...")
